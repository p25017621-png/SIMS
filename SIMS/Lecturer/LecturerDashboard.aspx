...
.stat-card{background:var(--white);border-radius:var(--radius);padding:22px 24px;box-shadow:var(--card-shadow);display:flex;align-items:center;gap:18px;border-left:4px solid transparent;transition:transform .25s,box-shadow .25s;}
...
.stat-info h2{font-size:30px;font-weight:700;color:var(--text);line-height:1;}
...
.section-title{font-size:17px;font-weight:700;color:var(--text);margin-bottom:18px;}
...
.card::after{content:'';position:absolute;top:0;right:0;width:70px;height:70px;background:linear-gradient(135deg,transparent 50%,var(--purple-pale) 50%);border-radius:0 var(--radius) 0 0;transition:opacity .25s;opacity:0;}
