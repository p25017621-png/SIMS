...
.search-input{padding:9px 16px 9px 36px;border:1.5px solid var(--border);border-radius:10px;font-family:'DM Sans',sans-serif;font-size:13px;background:var(--white);outline:none;width:200px;transition:all .2s;}
.search-input:focus{border-color:var(--purple);width:230px;}
...
.icon-btn{width:40px;height:40px;background:var(--white);border:1.5px solid var(--border);border-radius:12px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--muted);font-size:15px;position:relative;transition:all .2s;}
.icon-btn:hover{border-color:var(--purple);color:var(--purple);}
.notif-badge{position:absolute;top:-4px;right:-4px;width:18px;height:18px;background:#ef4444;border-radius:50%;color:#fff;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;border:2px solid var(--bg);}
.notif-panel{position:absolute;top:48px;right:0;width:300px;background:var(--white);border:1.5px solid var(--border);border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,.14);z-index:200;display:none;}
.notif-panel.show{display:block;}
.notif-header{padding:14px 18px;border-bottom:1px solid var(--border);font-size:14px;font-weight:700;}
.notif-item{padding:12px 18px;border-bottom:1px solid var(--border);display:flex;gap:10px;align-items:flex-start;}
.notif-item:last-child{border-bottom:none;}
.notif-dot{width:7px;height:7px;border-radius:50%;background:var(--purple);flex-shrink:0;margin-top:5px;}
.notif-item p{font-size:12px;color:var(--text);font-weight:600;line-height:1.4;}
...
.stat-card{background:var(--white);border-radius:var(--radius);padding:22px 24px;box-shadow:var(--card-shadow);display:flex;align-items:center;gap:18px;border-left:4px solid transparent;transition:transform .25s;}
...
.card{background:var(--white);border-radius:var(--radius);padding:26px 24px 22px;box-shadow:var(--card-shadow);display:flex;flex-direction:column;transition:transform .25s,box-shadow .25s;cursor:pointer;border:1px solid var(--border);position:relative;overflow:hidden;}
...
.card-icon{width:50px;height:50px;background:var(--purple-pale);border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:20px;color:var(--purple);transition:background .25s,color .25s;}
.card:hover .card-icon{background:linear-gradient(135deg,var(--purple),var(--purple-light));color:#fff;}
.card-arrow{color:var(--muted);font-size:13px;}
.card:hover .card-arrow{color:var(--purple);}
...
.dash-btn{display:inline-block;padding:10px 22px;background:linear-gradient(135deg,var(--purple),var(--purple-mid));color:#fff;border:none;border-radius:10px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer;transition:transform .2s;align-self:flex-start;box-shadow:0 4px 14px rgba(108,78,242,.35);}
.dash-btn:hover{transform:scale(1.04);}
.no-results{grid-column:1/-1;text-align:center;padding:40px;color:var(--muted);}
.no-results i{font-size:28px;margin-bottom:10px;display:block;}
...
<aside class="sidebar">
  ...
  <a href="LecturerDashboard.aspx" class="nav-item active"><i class="fa-solid fa-gauge-high"></i> Dashboard</a>
...
<main class="main">
  <div class="topbar">
    <div><h1>Dashboard</h1><p>Welcome back to SIMS</p></div>
    <div class="topbar-right">
      <div class="search-wrap">
        <i class="fa-solid fa-magnifying-glass search-icon"></i>
        <input type="text" class="search-input" id="searchBox" placeholder="Search quick access..." onkeyup="searchQuickAccess()"/>
      </div>
      <div class="bell-wrap">
        <div class="icon-btn" onclick="toggleNotif()">
          <i class="fa-regular fa-bell"></i>
          <span class="notif-badge"><asp:Label ID="lblNotifCount" runat="server" Text="0"/></span>
        </div>
        <div class="notif-panel" id="notifPanel">
          <div class="notif-header">Recent Announcements</div>
          <asp:Repeater ID="rptNotifications" runat="server">
            <ItemTemplate>
              <div class="notif-item">
                ...
                <div>
                  <p><%# Eval("title") %></p>
                  <span><%# Convert.ToDateTime(Eval("datePosted")).ToString("dd MMM yyyy") %></span>
                </div>
              </div>
            </ItemTemplate>
          </asp:Repeater>
          <asp:Panel ID="pnlNoNotif" runat="server" Visible="false">
            <div class="notif-empty"><i class="fa-regular fa-bell-slash"></i><br/>No announcements yet</div>
          </asp:Panel>
        </div>
      </div>
  ...
  <div class="hero">
    ...
    <div class="hero-graphic"><i class="fa-solid fa-chart-pie"></i></div>
  </div>
  <div class="stats">
    <div class="stat-card">
      <div class="stat-icon"><i class="fa-solid fa-users"></i></div>
      <div class="stat-info">
        <h2><asp:Label ID="lblTotalStudents" runat="server" Text="0"/></h2>
        <div class="label">Total Students</div>
        <div class="sub">Across all your courses</div>
      </div>
    </div>
    <div class="stat-card">
      <div class="stat-icon"><i class="fa-solid fa-calendar-check"></i></div>
      <div class="stat-info">
        <h2><asp:Label ID="lblTotalAttendance" runat="server" Text="0"/></h2>
    ...
    <div class="stat-card">
      ...
      <div class="stat-info">
        <h2><asp:Label ID="lblTotalMarks" runat="server" Text="0"/></h2>
  ...
  <div class="grid" id="quickAccessGrid">
    <div class="card" data-name="attendance manage attendance">
      ...
      <asp:Button ID="btnAttendance" runat="server" Text="Open Attendance" CssClass="dash-btn" OnClick="btnAttendance_Click"/>
    </div>
    <div class="card" data-name="marks manage marks grades">
      <div class="card-header">
      ...
      <asp:Button ID="btnMarks" runat="server" Text="Open Marks" CssClass="dash-btn" OnClick="btnMarks_Click"/>
    </div>
    <div class="card" data-name="students view students">
      <div class="card-header">
      ...
      <asp:Button ID="btnStudents" runat="server" Text="View Students" CssClass="dash-btn" OnClick="btnStudents_Click"/>
    </div>
    <div class="card" data-name="profile manage profile">
      <div class="card-header">
      ...
      <asp:Button ID="btnProfile" runat="server" Text="Manage Profile" CssClass="dash-btn" OnClick="btnProfile_Click"/>
    </div>
    <div class="card" data-name="courses assigned courses view courses">
      <div class="card-header">
      ...
      <asp:Button ID="btnCourses" runat="server" Text="View Courses" CssClass="dash-btn" OnClick="btnCourses_Click"/>
    </div>
    <div class="card" data-name="announcements post announcements">
      <div class="card-header">
      ...
      <asp:Button ID="btnAnnouncement" runat="server" Text="View Announcements" CssClass="dash-btn" OnClick="btnAnnouncement_Click"/>
    </div>
  </div>
  <div class="no-results" id="noResultsMsg" style="display:none;">
    <i class="fa-solid fa-magnifying-glass"></i>
    No quick access found matching your search.
  </div>
</main>
</form>
<script>
    function searchQuickAccess() {
        var val = document.getElementById('searchBox').value.toLowerCase().trim();
        var cards = document.querySelectorAll('#quickAccessGrid .card');
        var count = 0;
        cards.forEach(function (c) {
            var match = c.getAttribute('data-name').includes(val);
    ...
    document.addEventListener('click', function (e) {
        if (!e.target.closest('.bell-wrap')) document.getElementById('notifPanel').classList.remove('show');
    });
